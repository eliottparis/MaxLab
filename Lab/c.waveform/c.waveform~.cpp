...
void *waveform_new(t_symbol *s, int argc, t_atom *argv);
void waveform_free(t_waveform *x);
void waveform_assist(t_waveform *x, void *b, long m, long a, char *s);

t_max_err waveform_notify(t_waveform *x, t_symbol *s, t_symbol *msg, void *sender, void *data);
void waveform_getdrawparams(t_waveform *x, t_object *patcherview, t_jboxdrawparams *params);

void update_buffer_display(t_waveform *x);

// -- playing methods -- //

void waveform_position(t_waveform *x, double f);

// -- buffer's methods -- //

void waveform_normalize(t_waveform *x, double f);

// -- mousing methods -- //

void waveform_mouse_move(t_waveform *x, t_object *patcherview, t_pt pt, long modifiers);
void waveform_mouse_down(t_waveform *x, t_object *patcherview, t_pt pt, long modifiers);
void waveform_mouse_drag(t_waveform *x, t_object *patcherview, t_pt pt, long modifiers);

// -- attrs settings methods -- //

t_max_err waveform_setattr_buffername(t_waveform *x, t_symbol *s, long ac, t_atom* av);
t_max_err waveform_setattr_chanoffset(t_waveform *x, t_symbol *s, long ac, t_atom* av);
t_max_err waveform_setattr_number_of_points(t_waveform *x, t_symbol *s, long ac, t_atom* av);
t_max_err waveform_setattr_zoom_factor(t_waveform *x, t_symbol *s, long ac, t_atom* av);
t_max_err waveform_setattr_waveform_radius(t_waveform *x, t_symbol *s, long ac, t_atom* av);

// -- drawing methods -- //

void waveform_paint(t_waveform *x, t_object *view);
void draw_background(t_waveform *x, t_object *view, t_rect *rect);
void draw_waveform_full(t_waveform *x,  t_object *view, t_rect *rect);
void draw_waveform_sticks(t_waveform *x,  t_object *view, t_rect *rect);
void draw_horiz_waveform_sticks(t_waveform *x,  t_object *view, t_rect *rect);
void draw_cursor(t_waveform *x, t_object *view, t_rect *rect);
void draw_selection(t_waveform *x, t_object *view, t_rect *rect);

t_class *waveform_class;

int C74_EXPORT main()
{
	t_class *c;
    
	c = class_new("c.waveform~", (method)waveform_new, (method)waveform_free, (short)sizeof(t_waveform), 0L, A_GIMME, 0);
    
	c->c_flags |= CLASS_FLAG_NEWDICTIONARY;
	jbox_initclass(c, JBOX_COLOR | JBOX_FIXWIDTH);

	class_addmethod(c, (method)waveform_assist,				"assist",			A_CANT,	0);
	class_addmethod(c, (method)waveform_paint,				"paint",			A_CANT,	0);
	class_addmethod(c, (method)waveform_notify,				"notify",			A_CANT, 0);
	class_addmethod(c, (method)waveform_getdrawparams,		"getdrawparams",	A_CANT, 0);
	class_addmethod(c, (method)waveform_mouse_down,			"mousedown",		A_CANT, 0);
    class_addmethod(c, (method)waveform_mouse_move,			"mousemove",		A_CANT, 0);
	class_addmethod(c, (method)waveform_mouse_drag,			"mousedrag",		A_CANT, 0);
    class_addmethod(c, (method)waveform_mouse_down,			"mouseup",			A_CANT, 0);
	class_addmethod(c, (method)waveform_setattr_buffername, "set",				A_GIMME, 0);
	class_addmethod(c, (method)waveform_normalize,			"normalize",		A_FLOAT, 0);
	class_addmethod(c, (method)waveform_position,			"cursor",			A_FLOAT, 0);
	...
	CLASS_ATTR_RGBA                 (c, "bgcolor", 0, t_waveform, f_color_bg);
	CLASS_ATTR_STYLE                (c, "bgcolor", 0, "rgba");
	CLASS_ATTR_LABEL                (c, "bgcolor", 0, "Background Color");
	CLASS_ATTR_DEFAULT_SAVE_PAINT   (c, "bgcolor", 0, "0.95 0.95 0.95 1.");
    
    CLASS_ATTR_RGBA                 (c, "bdcolor", 0, t_waveform, f_color_bd);
	CLASS_ATTR_STYLE                (c, "bdcolor", 0, "rgba");
	CLASS_ATTR_LABEL                (c, "bdcolor", 0, "Border Color");
	CLASS_ATTR_DEFAULT_SAVE_PAINT   (c, "bdcolor", 0, "0.7 0.7 0.7 1.");
	
	CLASS_ATTR_RGBA                 (c, "cursorcolor", 0, t_waveform, f_color_cursor);
	...
	CLASS_ATTR_RGBA                 (c, "selectioncolor", 0, t_waveform, f_color_selection);
	...
	CLASS_ATTR_RGBA                 (c, "waveformcolor", 0, t_waveform, f_color_waveform);
	...
	CLASS_ATTR_LONG                 (c, "cornersize", 0, t_waveform, f_cornersize);
	...
	CLASS_ATTR_LONG					(c, "border", 0, t_waveform, f_bordersize);
	...
	CLASS_ATTR_DOUBLE				(c, "zoom", 0, t_waveform, f_zoom_factor);
	CLASS_ATTR_ACCESSORS            (c, "zoom", NULL, waveform_setattr_zoom_factor);
    CLASS_ATTR_LABEL                (c, "zoom", 0, "Zoom Factor");
    CLASS_ATTR_DEFAULT              (c, "zoom", 0, "1");
    CLASS_ATTR_SAVE                 (c, "zoom", 1);
	
	CLASS_ATTR_DOUBLE				(c, "wfradius", 0, t_waveform, f_waveform_radius);
	CLASS_ATTR_ACCESSORS            (c, "wfradius", NULL, waveform_setattr_waveform_radius);
    CLASS_ATTR_LABEL                (c, "wfradius", 0, "Waveform Radius");
    CLASS_ATTR_DEFAULT              (c, "wfradius", 0, "0.5");
    CLASS_ATTR_SAVE                 (c, "wfradius", 1);
	
	CLASS_ATTR_LONG                 (c, "wfdrawmode", 0, t_waveform, f_wf_draw_mode);
    CLASS_ATTR_LABEL                (c, "wfdrawmode", 0, "Waveform Drawing Mode");
    CLASS_ATTR_ENUMINDEX2           (c, "wfdrawmode", 0, "Horizontal", "Circular")
	CLASS_ATTR_DEFAULT              (c, "wfdrawmode", 0, "0");
	
	CLASS_ATTR_LONG                 (c, "sampsdrawmode", 0, t_waveform, f_samps_draw_mode);
    CLASS_ATTR_LABEL                (c, "sampsdrawmode", 0, "Samples Drawing Mode");
    CLASS_ATTR_ENUMINDEX2           (c, "sampsdrawmode", 0, "Full", "Sticks")
	CLASS_ATTR_DEFAULT              (c, "sampsdrawmode", 0, "0");
	
	CLASS_ATTR_LONG                 (c, "cursordrawmode", 0, t_waveform, f_cursor_mode);
	...
	CLASS_ATTR_SYM                  (c, "buffername", 0, t_waveform, f_buffername);
	CLASS_ATTR_CATEGORY             (c, "buffername", 0, "Behavior");
	CLASS_ATTR_ACCESSORS            (c, "buffername", NULL, waveform_setattr_buffername);
	CLASS_ATTR_LABEL                (c, "buffername", 0, "Buffer~ Object Name");
	CLASS_ATTR_DEFAULT              (c, "buffername", 0, "");
	CLASS_ATTR_SAVE                 (c, "buffername", 1);
	
    CLASS_ATTR_LONG                 (c, "chanoffset", 0, t_waveform, f_chanoffset);
    CLASS_ATTR_LABEL                (c, "chanoffset", 0, "Channel");
	CLASS_ATTR_ACCESSORS            (c, "chanoffset", NULL, waveform_setattr_chanoffset);
    CLASS_ATTR_DEFAULT              (c, "chanoffset", 0, "1");
    CLASS_ATTR_SAVE                 (c, "chanoffset", 1);
	
	CLASS_ATTR_LONG                 (c, "nbpoints", 0, t_waveform, f_number_of_points);
    CLASS_ATTR_LABEL                (c, "nbpoints", 0, "Number of Points");
	CLASS_ATTR_ACCESSORS            (c, "nbpoints", NULL, waveform_setattr_number_of_points);
...
void *waveform_new(t_symbol *s, int argc, t_atom *argv)
{
	...
	x = (t_waveform *)object_alloc(waveform_class);
	flags = 0 
			| JBOX_DRAWFIRSTIN 
			| JBOX_DRAWINLAST
			| JBOX_TRANSPARENT	
...
t_max_err waveform_notify(t_waveform *x, t_symbol *s, t_symbol *msg, void *sender, void *data)
{
	...
	if (msg == gensym("attr_modified"))
	{
		...
		else if (name == gensym("waveformcolor") || name == gensym("sampsdrawmode"))
		{
			jbox_invalidate_layer((t_object *)x, NULL, gensym("waveform_layer"));
		}
...
void waveform_paint(t_waveform *x, t_object *view)
{
	...
	if (x->f_wf_draw_mode == 0) // horizontal
	{
		draw_horiz_waveform_sticks(x, view, &rect);
	}
	
	else if (x->f_wf_draw_mode == 1)
	{
		//draw_selection(x, view, &rect);
		
		if (x->f_cursor_mode != 3)
		{
			if (x->f_samps_draw_mode == 0)
				draw_waveform_full(x, view, &rect);
			else
...
void draw_horiz_waveform_sticks(t_waveform *x,  t_object *view, t_rect *rect)
{
	...
	if (g)
	{
		...
		for (int i = 0; i < x->f_number_of_points; i++)
		{
			pt[0].x	= pt[1].x = ((double)i / (double)x->f_number_of_points) * (rect->width - (x->f_bordersize*2)) + x->f_bordersize + 2;
			
			pt[0].y = clip_minmax(centerHeight + (x->f_display_buffer[i]*zoom), x->f_bordersize*2, maxHeight);
			pt[1].y = clip_minmax(centerHeight - (x->f_display_buffer[i]*zoom), x->f_bordersize*2, maxHeight);
